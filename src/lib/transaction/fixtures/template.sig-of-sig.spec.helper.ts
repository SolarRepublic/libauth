...
export const sigOfSig: AuthenticationTemplate = {
  ...
  description:
    'An unusual example of a template which must be signed in a specific order: Second Signer may only sign after First Signer. This construction could be useful in scenarios where a particular signing order must be guaranteed, e.g. proving that a "most-responsible" party only signed for a transaction knowing they were the final signer.',
  entities: {
    signer_1: {
      name: 'First Signer',
      variables: {
